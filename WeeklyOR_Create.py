...
############################################################################################################
# Creating Orange Spain Weekly Jira
############################################################################################################
...
justify_reveneu.send_keys("Please execute the Orange Spain checklists")

add_description = wait.until(EC.presence_of_element_located((By.ID, "description")))
add_description.send_keys("Please execute the Orange Spain checklists")

click_create = wait.until(EC.presence_of_element_located((By.ID, "issue-create-submit")))
click_create.click()

# Waiting for Jira creation
jiracreated = wait.until(EC.presence_of_element_located((By.XPATH, '//h1[text()="[Orange Spain]-Weekly Sanity Checklist"]')))
